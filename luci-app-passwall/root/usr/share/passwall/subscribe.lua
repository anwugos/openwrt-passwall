...
-- 订阅 URL 里 '#' 及其后片段不会发送到服务端，统一剔除避免误配置
local function normalize_subscribe_url(url)
	if type(url) ~= "string" then
		return ""
	end
	url = api.trim(url)
	local idx = url:find("#", 1, true)
	if idx then
		url = url:sub(1, idx - 1)
	end
	return url
...
-- 兼容部分订阅站返回的 Xray JSON 配置数组，转换成 vless:// 链接后复用原解析流程
local function convert_xray_profile_to_vless(profile)
	if type(profile) ~= "table" then
		return nil
	end
	local outbounds = profile.outbounds
	if type(outbounds) ~= "table" then
		return nil
	end
	for _, ob in ipairs(outbounds) do
		if type(ob) == "table" and ob.protocol == "vless" then
			local vnext = ob.settings and ob.settings.vnext and ob.settings.vnext[1]
			local user = vnext and vnext.users and vnext.users[1]
			local uuid = user and user.id or nil
			local host = vnext and vnext.address or nil
			local port = vnext and vnext.port or nil
			if uuid and host and port then
				host = tostring(host):gsub("^%[", ""):gsub("%]$", "")
				local query = {}
				local stream = ob.streamSettings or {}
				if stream.network and stream.network ~= "" then
					query[#query + 1] = "type=" .. UrlEncode(tostring(stream.network))
				end
				if stream.security and stream.security ~= "" then
					query[#query + 1] = "security=" .. UrlEncode(tostring(stream.security))
				end
				if stream.network == "ws" and type(stream.wsSettings) == "table" then
					if stream.wsSettings.host and stream.wsSettings.host ~= "" then
						query[#query + 1] = "host=" .. UrlEncode(tostring(stream.wsSettings.host))
					end
					if stream.wsSettings.path and stream.wsSettings.path ~= "" then
						query[#query + 1] = "path=" .. UrlEncode(tostring(stream.wsSettings.path))
					end
				end
				if stream.security == "tls" and type(stream.tlsSettings) == "table" then
					if stream.tlsSettings.serverName and stream.tlsSettings.serverName ~= "" then
						query[#query + 1] = "sni=" .. UrlEncode(tostring(stream.tlsSettings.serverName))
					end
					if stream.tlsSettings.fingerprint and stream.tlsSettings.fingerprint ~= "" then
						query[#query + 1] = "fp=" .. UrlEncode(tostring(stream.tlsSettings.fingerprint))
					end
					if type(stream.tlsSettings.alpn) == "table" and #stream.tlsSettings.alpn > 0 then
						query[#query + 1] = "alpn=" .. UrlEncode(table.concat(stream.tlsSettings.alpn, ","))
					end
					if stream.tlsSettings.allowInsecure ~= nil then
						query[#query + 1] = "allowInsecure=" .. ((stream.tlsSettings.allowInsecure == true) and "1" or "0")
					end
				end
				local link = UrlEncode(tostring(uuid)) .. "@" .. host .. ":" .. tostring(port)
				if #query > 0 then
					link = link .. "?" .. table.concat(query, "&")
				end
				local remarks = profile.remarks or ""
				if remarks ~= "" then
					link = link .. "#" .. UrlEncode(tostring(remarks))
				end
				return "vless://" .. link
			end
		end
...
local function parse_link(raw, add_mode, group, cfgid)
	if raw and #raw > 0 then
		local nodes, szType
		local node_list = {}
		local raw_trim = api.trim(raw)
		-- JSON 数组订阅（如 app=xray 返回整份配置）兼容处理
		if raw_trim:sub(1, 1) == "[" and raw_trim:find("\"outbounds\"", 1, true) then
			local arr = jsonParse(raw_trim)
			if type(arr) == "table" and #arr > 0 then
				nodes = {}
				for _, profile in ipairs(arr) do
					local link = convert_xray_profile_to_vless(profile)
					if link then
						nodes[#nodes + 1] = link
					end
				end
		...
		-- SSD 似乎是这种格式 ssd:// 开头的
		if not nodes and raw:find('ssd://') then
			szType = 'ssd'
...
local execute = function()
	do
		...
		for index, value in ipairs(subscribe_list) do
			...
			local url = normalize_subscribe_url(value.url or "")
